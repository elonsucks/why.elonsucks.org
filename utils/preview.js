...
(async () => {
    ...
    require('child_process').exec(`http-server -p ${availPort}`).stdout.on('data', data => {
        if (data.includes('Hit CTRL-C to stop the server')) { // server ready msg, enrich then preview
            data = data.replace(/Available on:[\s\S]*/, `Listening at ${by}http://localhost:${availPort}${nc}\n\n`)
