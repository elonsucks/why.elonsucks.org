(async()=>{let urls={all:function extractURLs(obj){let urls=[];return Array.isArray(obj)?obj.forEach(entry=>{"string"==typeof entry?urls.push(entry):urls.push(...extractURLs(entry))}):obj.url?urls.push(obj.url):Object.values(obj).forEach(val=>urls.push(...extractURLs(val))),urls}(await(await fetch("assets/data/urls.json")).json()),visited:JSON.parse(localStorage.weso_visitedURLs||"[]")},randURL=(urls.unvisited=urls.all.filter(url=>!urls.visited.includes(url)),urls.unvisited.length||(urls.unvisited.push(...urls.all),localStorage.weso_visitedURLs="[]"),urls.unvisited[Math.floor(Math.random()*urls.unvisited.length)]);if(location.search.startsWith("?debug")){let debugOutput=`<pre>Redirect URL (#${urls.unvisited.indexOf(randURL)+1} of ${urls.unvisited.length} unvisited):
