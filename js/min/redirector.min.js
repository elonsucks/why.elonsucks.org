...
`+`<strong><a href="${urls.random}">${urls.random}</a></strong></pre>`;location.search.endsWith("=all")&&["unvisited","visited"].forEach(urlsType=>{var numberedURLs=JSON.stringify(urls[urlsType],void 0,2).replace(/"http/g,(match,offset,string)=>(string.slice(0,offset).match(/"http/g)||[]).length+1+'. "'+match.slice(1));debugOutput+=`<pre>urls.${urlsType} = ${numberedURLs.replace(new RegExp(`\\d+\\. "${urls.random.replace(/[.*+?^=!:${}()|[\]\/\\]/g,"\\$&")}"`),'<strong style="color: #48b720">$&</strong>')}</pre>`}),document.body.innerHTML=debugOutput}else localStorage.visitedURLs=JSON.stringify([...urls.visited,urls.random]),document.location=urls.random})();